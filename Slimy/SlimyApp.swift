...
class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    var viewModels: [String: SlimyViewModel] = [:] // UUID -> SlimyViewModel
    var window: NSWindow?
    let vm: SlimyViewModel = .init()
    ...
    private func enableSkyLightOnAllWindows() {
        if Defaults[.showOnAllDisplays] {
            windows.values.forEach { window in
                if let skyWindow = window as? SlimyNotchSkyLightWindow {
        ...
        Task {
            ...
            await MainActor.run {
                if Defaults[.showOnAllDisplays] {
                    ...
                    if let skyWindow = self.window as? SlimyNotchSkyLightWindow {
                        skyWindow.disableSkyLight()
                    }
                }
            }
    ...
    private func createSlimyNotchWindow(for screen: NSScreen, with viewModel: SlimyViewModel) -> NSWindow {
        let rect = NSRect(x: 0, y: 0, width: windowSize.width, height: windowSize.height)
        let styleMask: NSWindow.StyleMask = [.borderless, .nonactivatingPanel, .utilityWindow, .hudWindow]
        
        let window = SlimyNotchSkyLightWindow(contentRect: rect, styleMask: styleMask, backing: .buffered, defer: false)
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        if !Defaults[.showOnAllDisplays] {
            ...
            let window = createSlimyNotchWindow(
    ...
    @objc func adjustWindowPosition(changeAlpha: Bool = false) {
        if Defaults[.showOnAllDisplays] {
            ...
            for screen in NSScreen.screens {
                ...
                if windows[uuid] == nil {
                    let viewModel = SlimyViewModel(screenUUID: uuid)
                    let window = createSlimyNotchWindow(for: screen, with: viewModel)
            ...
            if window == nil {
                window = createSlimyNotchWindow(for: selectedScreen, with: vm)
            }
